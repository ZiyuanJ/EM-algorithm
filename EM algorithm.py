...
class EM:
    ...
    def __init__(self, k ):
        ...
        #self.tol = tol ## threshold to stop `epsilon`
    ...
    def fit_EM(self, X, max_iters ):
        ...
        ## as  of the points from datasets        
        ...
        #define P(x|y=j) as function of parameter vector lambda
		P = lambda mu, s:  (2 * np.pi*s) ** (-X.shape[1]/2.) \
                * np.exp(-.5 *  np.einsum('ij,ij->ij',X[:,]-mu,X[:,]-mu) /s) 
        ...
        while len(log_likelihoods) < max_iters:
            ...
            for k in range(self.k):
                nj[k] = w[k] * P(mu[k], np.diag(Sigma)[k])
            ...
            ## Normalize so that the responsibility matrix is row stochastic, which is P (y=j|xi)
            R = (nj / np.sum(nj, axis = 0))
            ...
            for k in range(self.k):
                ...
                ## means
                mu[k] = 1. / R[k] * np.sum(R[k] * X.T, axis = 1).T
                x_mu = np.matrix(X - mu[k])
                
                ## covariances
                Sigma[k] = np.array(1 / n_js[k] * np.dot(np.multiply(x_mu.T,  R[:, k]), x_mu))
